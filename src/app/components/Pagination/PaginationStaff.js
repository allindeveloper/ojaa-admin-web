...
class PaginationStaff extends React.Component {
    constructor(props) {
        super(props);
    ...
    setPage(page) {
      ...
      this.props.service(this.props.controller,page).then(res=>{
           // console.log('back from search')
            ...
            if (page > 2) {
                data=res.data.staffs.map((item,index)=>{item.no= (1+index) +((page-1) * this.state.pageSize); return item});
            }
            else if(page ===2){
                data=res.data.staffs.map((item,index)=>{item.no= 1+index + this.state.pageSize; return item});
                
            }
            else{
                data=res.data.staffs.map((item,index)=>{item.no= ++index ; return item});
            }
